...
/**
 * This class is the one that accesses the FFLocation web service. Provides
 * access to all the API functions in the web service the android needs. It also
 * has a cache system not to overload the system or the net. This kind of cache
 * will also respond faster and make the android programmer not to have to worry
 * about the net overloading.
 */
public class ToServer {
	...
	/**
	 * This process authenticates the user in the server and stores the received
	 ...
	 * @throws Exception
	 *             if something fails, for example, connection to the server and
	 *             soap serialization.
	 */
	public static boolean login(String n, String p) throws Exception {
		...
		try {
			...
			envelope.addMapping("http://controller", "Auth",
					new KAuth().getClass());
	...
	/**
	 * This process authenticates the user in the server and stores the received
	 * authentication object (For running only in Tomcat).
	 * 
	 * @param n
	 *            User name
	 * @param p
	...
	/**
	 * This method return the current Auth, if the method warns that the current
	 * session has expired we do another login
	 * 
	 * @return the Auth
	 * @throws Exception
	 *             if something fails.
	 ...
	 * @throws Exception
	 *             if something fails, for example, connection to the server and
	 *             soap serialization. Other exception thrown is when not
	 *             logged.
	 */
	...
	/**
	 * Gets one note from the server if you have rights to do it.
	 * 
	 * @param noteID
	 * @return The note with the photo attached, if it has. null if you have no
	 *         rights to get it.
	 * @throws Exception
	 *             if something fails, for example, connection to the server and
	 *             soap serialization. Other exception thrown is when not
	 ...
	 *            The users id.
	 * @param c
	 *            The n count of positions.
	 * @return The list of positions null if you have no rights to get it.
	 * @throws Exception
	 *             if something fails, for example, connection to the server and
	 *             soap serialization. Other exception thrown is when not
	 *             logged.
	 ...
	 * 
	 * @param u
	 *            The new user's information object
	 * @param pw
	...
	/**
	 * This function allows the programmer to change the authenticated user's
	 * information.
	 * 
	 * @param u
	...
	/**
	 * This function is used to search for users in the system that are not
	 * already you friends.
	 * 
	 * @param nick
	 *            String the text should contain
	 * @param name
	 *            String the name should contain
	 * @param surname
	 *            String the surname should contain
	 * @param country
	 *            String the country should contain
	...
	/**
	 * Sends a new note to the server. Attaching the photo is optional.
	 * 
	 * @param longitude
	 *            of the GPS.
	 * @param latitude
	 ...
	 * @param photo
	 *            The photo to be attached or null if none.
	 * @return whether the process succeed.
	 * @throws Exception
	 *             if something fails, for example, connection to the server and
	 *             soap serialization. Other exception thrown is when not
	 ...
	 */
	/*
	 * public static boolean setNote(float longitude, float latitude, String
	 * note, Photo photo) throws Exception { if(getAuth()==null) throw new
	 * Exception("Not logged"); KAuth a = getAuth(); String METHOD_NAME =
	 * "setNote"; SoapObject request = new SoapObject(NAMESPACE, METHOD_NAME);
	 * 
	 * KPosition p = new KPosition(); p.setLatitude(latitude);
	 * p.setLongitude(longitude);
	 * 
	 * request.addProperty("a", a); request.addProperty("position", p);
	 * request.addProperty("note", note); if(photo!=null){
	 * request.addProperty("photo", Base64.encodeToString(photo.getPhoto(),
	 * Base64.DEFAULT)); }else{ request.addProperty("photo", "null"); }
	 * 
	 * SoapSerializationEnvelope envelope = new SoapSerializationEnvelope(
	 * SoapEnvelope.VER11); envelope.setOutputSoapObject(request);
	 * 
	 * envelope.addMapping("http://controller", "KAuth", KAuth.class);
	 * envelope.addMapping("http://controller", "KPosition", KPosition.class);
	 * 
	 ...
	 *            The user's id.
	 * @param c
	 *            The n count of notes wanted.
	 * @param attachPhotos
	 *            Whether you want the photos attached to the notes.
	 ...
	 *             logged.
	 */
	public static List<KNote> getNotes(int id, int c, boolean attachPhotos)
			throws Exception {
		if (getAuth() == null)
			throw new Exception("Not logged");
		long currentTime = System.currentTimeMillis();

		if (listNotes == null || selectedUserID != id
				|| (currentTime - notesTime) > cacheLive) {
	...
	/**
	 * This function returns the list of users that have asked the logged user
	 * to be friends.
	 * 
	...
	/**
	 * Simple hash function using the SHA1 algorithm. This is used because the
	 * server specifies that the password must be hashed with SHA1.
	 * 
	 * @param message
	 *            to be hashed.
	 * @return sha1(message)
	 * @throws NoSuchAlgorithmException
	 *             if the the algorithm is not available in the system.
	 */
