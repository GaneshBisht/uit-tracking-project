...
public class FFLocationAPI {
	...
	/**
	 * Logs in a user into the authentication system. This log in function does not allow administrators.
	 * 
	 * @param nick
	 *            is user's nick.
	 * @param pw
	 *            is the user's password hashed with sha1.
	 * @return the authentication object or null if failed.
	 */
	public Auth login(String nick, String pw) {
		User u = dbc.loadUser(nick);
		if (u == null || u.isAdministrator())
			return null;
	 ...
	 * 
	 * @param nick
	 *            is user's or admin's nick.
	 * @param pw
	 *            is the user's or admin's password hashed with sha1.
	 * @return the authentication object or null if failed.
	 */
	...
	/**
	 * Gets the last n position of a user. The user must be administrator or friend of the target to have rights to get this.
	 * 
	 * @param a
	 *            the authentication object
	 * @param id
	...
	public Position[] getPositions(Auth a, int id, int c) {
		...
		if (uid == null)
			return null;
		if (!dbc.loadUser(uid).isAdministrator() && !dbc.areFriends(uid, id))
			return null;
	...
	public boolean newAdmin(Auth a, UserInfo ui, String pw) {
		if (!dbc.loadUser(a.getUserID()).isAdministrator())
			return false;
		User u = new User(ui);
		u.setPassword(pw);
		if (dbc.newUser(u)) {
	 ...
	 *            the authentication object
	 * @param p
	 *            the position object to be stored
	 ...
	 *            the authentication object
	 * @param position
	 *            the position object containing the latitude and longitude
	 * @param note
	 *            the text of the note
	 * @param photo
	 *            the photo if there is one, otherwise null
	 * @return whether the process succeed
	 */
	...
	/**
	 * This function returns the last n notes of an specific user. This notes can be requested with or with out photos.
	 * 
	 * @param a
	 *            the authentication object
	 * @param id
	 ...
	 * @param c
	 *            the n count of notes
	 * @param attachPhotos
	 *            whether you need the photos.
	...
	public Note[] getNotes(Auth a, int id, int c, boolean attachPhotos) {
		...
		if (uid == null)
			return null;
		if (!dbc.areFriends(uid, id) || dbc.loadUser(a.getUserID()).isAdministrator())
			return null;
	...
	public UserInfo[] getUsers(Auth a, int count, int page) {
		...
		if (uid == null)
			return null;
		if (!dbc.loadUser(uid).isAdministrator())
			return null;
	 ...
	 *            the authentication object
	 * @param nick
	 *            text the nick should contain
	 * @param name
	 *            text the name should contain
	 * @param surname
	 *            text the surname should contain
	 * @param country
	 *            text the country should contain
	...
	public Note getNote(Auth a, int noteID) {
		...
		if (!dbc.areFriends(n.getOwner(), id) && !dbc.loadUser(id).isAdministrator())
			return null;
		return new SNote(n, true);
	}

	/**
	 * This function calculates the hash code of a String into another one. It uses the SHA1 algorithm. The code has been modified, but the original can be
	 * found at the url "http://es.debugmodeon.com/articulo/generar-hash-sha-1-de-una-cadena-en-java"
	 * 
	 * @param message
	 *            String to be hashed
	 * @return SHA1(message)
	...
	public Auth newUser1(UserInfo ui, String pw, String photo) {
		if (photo.equals("null"))
			photo = null;
		User u = new User(ui);
		u.setPassword(pw);
